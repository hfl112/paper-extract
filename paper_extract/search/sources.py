...
def select_sources(names: list[str] | None) -> list[Source]:
    ...
    Note: arXiv is not a search source — OpenAlex already indexes arXiv, so those
    papers arrive through it. arXiv full text is fetched by a dedicated adapter in
    the fetch layer (keyed on the 10.48550/arXiv.* DOI), like bioRxiv."""
    if not names:
